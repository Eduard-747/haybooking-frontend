...
export function HeroSection({ onSearch, onCategorySelect, activeCategory = "All" }: HeroSectionProps) {
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center pt-8 md:pt-12 pb-4 px-4 text-center overflow-hidden">
      ...
      {/* Search Bar */}
      <form onSubmit={handleSearch} className="w-full max-w-2xl relative mb-8 flex items-center px-1">
        <div className="absolute left-5 sm:left-5 top-1/2 -translate-y-1/2 text-muted-foreground">
          <Search className="h-5 w-5" />
        </div>
        <Input 
          type="text" 
          placeholder={t("landing.searchPlaceholder", "Search for services, businesses, or locations...") as string} 
          className="w-full h-11 sm:h-12 pl-11 sm:pl-12 pr-20 sm:pr-28 rounded-full border-border/60 shadow-sm text-sm focus-visible:ring-1 focus-visible:ring-[#BC9B9E]"
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
        />
        <button 
          type="submit"
          className="absolute right-2.5 sm:right-2.5 top-1/2 -translate-y-1/2 px-3 sm:px-5 py-1.5 bg-[#BC9B9E] text-white rounded-full text-xs sm:text-sm font-medium hover:bg-[#a68689] transition-colors"
        >
